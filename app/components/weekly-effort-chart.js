...
export default Component.extend({
  ...
  percentdelivered: computed(
    "tangiblelaborthisweek",
    "committedhours",
    function() {
      let actualhours = parseFloat(this.get("tangiblelaborthisweek")).toFixed(
        2
      );
  ...
  color: computed("tangiblelaborthisweek", "committedhours", function() {
    ...
    let committedhours = parseFloat(this.get("weeklyCommitted")).toFixed(2);
    let percentdelivered = parseFloat(
      (actualhours * 100) / committedhours
    ).toFixed(2);
    return percentdelivered <= 30
      ? "red"
      : percentdelivered > 30 && percentdelivered <= 90
      ? "orange"
      : "green";
  }),
  ...
  didReceiveAttrs() {
    ...
    let enddate = moment()
      .endOf("week")
      .format();
    return this.get("dashboardService")
      .getWeeklyEffort(forUserId, startdate, enddate)
      .then(result => {
