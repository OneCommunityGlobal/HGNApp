...
export default Component.extend({
  ...
  sortedProjectMembers: Ember.computed.sort(
    "projectmembers",
    "sortMembersByName"
  ),
  ...
  init: function() {
    ...
    $.each(memberdata, function(key, value) {
      ...
      for (var i in temp) {
        ...
        let tempobj = {};
        tempobj["date"] = moment(temp[i][0].dateOfWork).week();
        tempobj["hours"] = hours;
      ...
      for (var i in finaltemphourdata) {
        ...
        let finaltempobj = {};
        finaltempobj[finaltemphourdata[i][0].date] = (temphours / 3600).toFixed(
          2
        );
      ...
      for (i = 1; i < sendlinedata.length; i++) {
        ...
        for (var x = 0; x < deletelinedata[item].length; x++) {
          ...
          if (Object.keys(deletelinedata[item][x])[0] == sendlinedata[i][0]) {
            sendlinedata[i].push(
              parseInt(
                deletelinedata[item][x][Object.keys(deletelinedata[item][x])[0]]
              )
            );
    ...
    let tempdata = [["Name", "Prior Week", "Selected Period"]];
    let tempdataid = [];
    let tempContributed = [];
    $.each(memberdata, function(key, value) {
      let temp = [];
    ...
    for (var i = 0; i < tempdata.length; i++) {
      //Admin, Core
      for (var name in tempmembers) {
        //Swathy, Jaem, Admin, Core
  ...
  didInsertElement() {
    ...
    function drawBarChart(senddata) {
      ...
      for (var i = 1; i < numRows; i++) dataTable.addRow(senddata[i]);
      dataTable.sort([
        {
          column: 1,
          desc: true
    ...
    function drawPieChart(piedata) {
      ...
      var chart = new google.visualization.PieChart(
        document.getElementById("member_pie")
      );
    ...
    function drawLineChart(sendlinedata) {
      ...
      for (var i = 1; i < numRows; i++) dataTable.addRow(sendlinedata[i]);
      var columnsTable = new google.visualization.DataTable();
      columnsTable.addColumn("number", "colIndex");
      columnsTable.addColumn("string", "colLabel");
      var initState = {
